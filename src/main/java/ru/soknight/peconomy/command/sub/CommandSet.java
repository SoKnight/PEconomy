...
public class CommandSet extends ExtendedSubcommandExecutor {
	...
	public CommandSet(DatabaseManager databaseManager, CurrenciesManager currenciesManager,
		...
		Validator permval = new PermissionValidator("peco.command.set", permmsg);
	...
	public void executeCommand(CommandSender sender, String[] args) {
		...
		if(limit != 0 && amount > limit) {
			...
			messages.sendFormatted(sender, "set.limit",
					"%currency%", symbol,
					"%limit%", limitstr);
			return;
		}
		
		// Checking for balance already
		...
		// Saving transaction
		Transaction transaction = new Transaction(owner, currencyid, TransactionType.SET, pre, amount, source);
		databaseManager.saveTransaction(transaction);
		
		int id = transaction.getId();
		
		// Sending messages to sender and wallet owner if he is online
		messages.sendFormatted(sender, "set.other",
				"%currency%", symbol,
				"%player%", owner,
				"%from%", prestr,
				"%operation%", operation,
		...
		if(offlinetarget.isOnline())
			messages.sendFormatted(offlinetarget.getPlayer(), "set.self",
					"%currency%", symbol,
					"%player%", owner,
					"%from%", prestr,
					"%operation%", operation,
